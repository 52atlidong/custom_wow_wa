...
function TT:AddLineForID(id, linkType, noadd)
	...
	if linkType == types.item then
		...
		if iconID then
			-- add icon id
			self:AddDoubleLine("图标ID:", format(DB.InfoColor.."%s|r", iconID))
		end
	end

	if linkType == types.spell then
		local info = C_Spell.GetSpellInfo(id)
		if info and info.iconID then
			self:AddDoubleLine("图标ID:", format(DB.InfoColor.."%s|r", info.iconID))
		end
	end
...
-- cp ndui_support/TooltipID.lua C:\game\wow\World of Warcraft\_retail_\Interface\AddOns\NDui\Modules\Tooltip\TooltipID.lua